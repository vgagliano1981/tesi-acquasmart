...
Durante i test eseguiti, la piattaforma cloud ha elaborato decine di migliaia di transazioni telemetriche MQTT al minuto senza riscontrare colli di bottiglia (bottlenecks). Sotto stress, la perfetta sinergia tra l'intercettazione alleggerita del broker, il backend asincrono (garantito dall'architettura ASGI di FastAPI) e la fluidità della libreria scikit-learn, ha offerto risultati prestazionali di altissimo profilo. 

L'intero ciclo logico del dato — comprensivo della ricezione dal topic, del parsing JSON, dell'estrazione delle caratteristiche (feature extraction), dello scoring e predizione tramite Isolation Forest, e della finalizzazione asincrona sul database SQLite/PostgreSQL (commit) — si è completato sistematicamente all'interno di un ristretto lasso temporale misurato in **50-80 millisecondi** a record.

Dal punto di vista dell'ottimizzazione del footprint in memoria (elemento vitale qualora si decidesse di spostare l'inferenza su gateway fisici tramite Edge Computing), le routine di ri-addestramento (re-training) del modello sono risultate straordinariamente snelle. Il modello è stato impostato per aggiornare i propri rami decisionali ogni 100 nuove letture, scartando i dati più vecchi di 10.000 record per evitare la saturazione della RAM. Ciò conferma come *AcquaSmart* sia una soluzione altamente scalabile, capace di accomodare un immenso numero di dispositivi fisici collegati in simultanea con investimenti irrisori a carico dei servizi server (Cloud Hosting).
